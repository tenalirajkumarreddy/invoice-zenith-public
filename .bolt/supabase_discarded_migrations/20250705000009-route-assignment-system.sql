-- Create route_assignments table for tracking route assignments
...
-- Create route_orders table for tracking orders during route
...
-- Function to generate route assignment ID
...
-- Function to update route order timestamps
...
INSERT INTO public.products (product_code, product_name, description, category, unit_price, cost_price, tax_rate) VALUES
('PROD-001', 'LED Bulb 9W', 'Energy efficient LED bulb', 'Lighting', 120.00, 80.00, 18.00),
('PROD-002', 'Switch Board', 'Electrical switch board', 'Electrical', 45.00, 30.00, 18.00),
('PROD-003', 'Wire 1.5mm', 'Copper wire 1.5mm', 'Electrical', 180.00, 120.00, 18.00),
('PROD-004', 'MCB 16A', 'Miniature Circuit Breaker', 'Electrical', 85.00, 55.00, 18.00),
('PROD-005', 'Socket 5A', 'Electrical socket 5A', 'Electrical', 35.00, 25.00, 18.00);
...
CREATE INDEX idx_route_assignments_date ON public.route_assignments(assigned_date);
CREATE INDEX idx_route_orders_route_assignment_id ON public.route_orders(route_assignment_id);
CREATE INDEX idx_route_orders_customer_id ON public.route_orders(customer_id);
