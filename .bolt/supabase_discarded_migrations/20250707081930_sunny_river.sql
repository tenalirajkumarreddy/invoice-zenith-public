/*
  ...
  1. New Tables
    - `customer_transactions`
      ...
      - `agent_id` (uuid, optional foreign key to auth.users)
      - `created_at` (timestamp)
      - `metadata` (jsonb, optional)

  2. Security
    - Enable RLS on `customer_transactions` table
    - Add policies for users to view and insert transactions
...
BEGIN
  ...
  -- Add invoice foreign key constraint
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'invoices') THEN
    ALTER TABLE public.customer_transactions 
    ADD CONSTRAINT customer_transactions_invoice_id_fkey 
  ...
  -- Add agent foreign key constraint
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
    ALTER TABLE public.customer_transactions 
    ADD CONSTRAINT customer_transactions_agent_id_fkey 
